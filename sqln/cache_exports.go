...
// NewCache re-exportado de cache/ para backward compatibility.
func NewCache[T any](name string, ttl time.Duration) *Cache[T] {
	return cache.NewCache[T](name, ttl)
}

// InstanceRedis re-exportado de cache/ para backward compatibility.
func InstanceRedis() redis.UniversalClient {
	return cache.InstanceRedis()
}

// NewCacheRedis re-exportado de cache/ para backward compatibility.
func NewCacheRedis() {
	cache.NewCacheRedis()
}
